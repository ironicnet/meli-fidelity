...
class sfGuardRouting
{
  ...
  static public function addRouteForForgotPassword(sfEvent $event)
  {
    ...
    $r->prependRoute('sf_guard_forgot_password', new sfRoute('/forgot-password', array('module' => 'sfGuardForgotPassword', 'action' => 'index')));
    $r->prependRoute('sf_guard_forgot_password_change', new sfDoctrineRoute('/forgot-password/:unique_key', array(
  ...
  {
    $event->getSubject()->prependRoute('sf_guard_register', new sfRoute('/register', array('module' => 'sfGuardRegister', 'action' => 'index'))); 
  }
}