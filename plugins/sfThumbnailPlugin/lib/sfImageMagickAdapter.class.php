...
class sfImageMagickAdapter
{
  ...
  public function __construct($maxWidth, $maxHeight, $scale, $inflate, $quality, $options, $crop, $cropVAlign, $cropAlign)
  {
    ...
    $this->crop = $crop;
    $this->cropVAlign = $cropVAlign;
    $this->cropAlign = $cropAlign;    
  }
  ...
  public function loadFile($thumbnail, $image)
  {
    ...
    $thumbnail->initThumb($this->sourceWidth, $this->sourceHeight, $this->maxWidth, $this->maxHeight, $this->scale, $this->inflate, $this->crop, $this->cropVAlign, $this->cropAlign);
  ...
  {
    $command = ' -thumbnail ';
    //claculate offsets & generate downsized image
    if ($this->crop) {
      if ($thumbnail->getSourceX()>=$thumbnail->getSourceY()) {
        $command .= 'x'.$thumbnail->getThumbHeight();
        $tmpWidth = ($this->sourceWidth/$this->sourceHeight)*$thumbnail->getThumbHeight();
        $offsetX = $thumbnail->getSourceX()*($tmpWidth/$this->sourceWidth);
        $offsetY = 0; 	
      }else{
        $command .= $thumbnail->getThumbWidth().'x';
        $tmpHeight = ($this->sourceWidth/$this->sourceHeight)*$thumbnail->getThumbWidth(); 
        $offsetY = $thumbnail->getSourceY()*($tmpHeight/$this->sourceHeight);
        $offsetX = 0; 	         
      }
    }else{
    ...
    // crop the downsizeimage
    if($this->crop){
     $command = ' -crop ';  
     $command .= $thumbnail->getThumbWidth().'x'.$thumbnail->getThumbHeight().'+'.$offsetX.'+'.$offsetY;
     exec($this->magickCommands['convert'].' '.$command.' '.escapeshellarg($thumbDest).' '.escapeshellarg($thumbDest));     	
