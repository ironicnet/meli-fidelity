 ...
 * @author     Tobias Stanzel <t.stanzel@bgcc.at>
 */
class sfThumbnail
{
  /**
  ...
  */
  ...
  /**
  * Offset of source image in pixels
  */  
  protected $sourceX;
  /**
  * Offset of source image in pixels
  */  
   ...
   */
  public function __construct($maxWidth = null, $maxHeight = null, $scale = true, $inflate = true, $quality = 75, $adapterClass = null, $adapterOptions = array(), $crop = false, $cropVAlign=null, $cropAlign=null)
  {
    if (!$adapterClass)
    {
    ...
    $this->adapter = new $adapterClass($maxWidth, $maxHeight, $scale, $inflate, $quality, $adapterOptions, $crop, $cropVAlign, $cropAlign);
  }
  ...
  */
  ...
  /**
   * Returns the x offset of the source
   */
  ...
  /**
   * Returns the y offset of the source
   */
   ...
   * Used by adapter
   */
  public function initThumb($sourceWidth, $sourceHeight, $maxWidth, $maxHeight, $scale, $inflate, $crop, $cropVAlign, $cropAlign)
  {
    if ($maxWidth > 0)
    {
    ...
    if ($crop && isset($ratioWidth) && isset($ratioHeight))
    {
      if($ratioWidth>$ratioHeight){
        $originalsourceHeight =$this->sourceHeight;
        $this->sourceHeight = $maxHeight / $ratioWidth;
        switch ($cropVAlign){
          case 'top':
            $this->sourceY = 0 ;
            break;
          case 'bottom':
            $this->sourceY = ($originalsourceHeight - $this->sourceHeight);
            break;
          case 'center':
          default:
            $this->sourceY = ($originalsourceHeight - $this->sourceHeight)/2;
        }
       
      }else{
        $originalsourceWidth =$this->sourceWidth;
        $this->sourceWidth = $maxWidth / $ratioHeight;
        switch ($cropAlign){
          case 'left':
            $this->sourceX = 0 ;
            break;
          case 'right':
            $this->sourceX = ($originalsourceWidth - $this->sourceWidth);
            break;
          case 'middle':
          default:
            $this->sourceX = ($originalsourceWidth - $this->sourceWidth)/2;
        }  
  
      }
      $this->thumbWidth = floor($maxWidth);
      $this->thumbHeight = floor($maxHeight); 
    }elseif ($scale)
    {
