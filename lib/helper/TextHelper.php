 ...
 */
function truncate_text($text, $length = 30, $truncate_string = '...', $truncate_lastspace = false, $cut_lastword = true)
{
    if ($text == '')
    {
        return '';
    ...
    {
        $truncate_length = $length - strlen($truncate_string);
        if ($cut_lastword)
        {
            while ($text[$truncate_length] != ' ' && $truncate_length > 0)
                $truncate_length--;
        }
        $truncate_text = substr($text, 0, $truncate_length);
...
function highlight_text($text, $phrase, $highlighter = '<strong class="highlight">\\1</strong>')
{
    ...
    if (is_array($phrase) or ($phrase instanceof sfOutputEscaperArrayDecorator))
    {
        ...
        {
            $pattern[] = '/\b(' . preg_quote($word, '/') . ')\b/ie';
            $replacement[] = $highlighter;
        }
    }
    else
    {
        $pattern = '/\b(' . preg_quote($phrase, '/') . ')\b/ie';
        $replacement = $highlighter;
    }
